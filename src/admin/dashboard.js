...
import renderApproved from './approvedStories'
import renderPending from './PendingStories'
...
const manageStory = () => {
	document.querySelectorAll('.manage-stories').forEach(btn => {
		btn.addEventListener('click', e => {
			const myStoryId = e.target.id
			const activeSection = document.querySelector(`.${myStoryId}`)
			const activeStoryBtn = document.querySelectorAll('.manage-stories')
			const activeStory = document.querySelector(`#${myStoryId}`)
